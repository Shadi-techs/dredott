...
export default function PropertyDetailPage() {
  ...
  const handleWhatsApp = () => {
    if (!user) {
      router.push(`/${locale}/login?redirect=/${locale}/stays/${slug}&reason=contact`)
      return
    }
  ...
  if (notFound) return (
    <div style={{ minHeight: '100vh', background: '#FAF9F6' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '70vh', gap: 16 }}>
        ...
        <button onClick={() => router.push(`/${locale}/stays`)} style={{ padding: '10px 24px', background: '#2C3A6B', color: '#D4A843', borderRadius: 10, border: 'none', cursor: 'pointer', fontWeight: 600 }}>
