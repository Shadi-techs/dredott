      ...
      <div style={{ position: 'relative', paddingTop: 64, overflow: 'hidden' }}>
        <div style={{ background: 'linear-gradient(135deg, #0e1428 0%, #1a2440 50%, #0d2b26 100%)', padding: '40px 24px 48px', position: 'relative' }}>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1, textAlign: isRTL ? 'right' : 'left' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13, letterSpacing: '0.12em', fontFamily: "'JetBrains Mono', monospace", margin: '0 0 16px' }}>
              {tx.hero_sub}
            </p>
            {!loading && properties.length > 0 && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(212,168,67,0.12)', border: '1px solid rgba(212,168,67,0.3)', borderRadius: 24, padding: '6px 14px' }}>
                <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#4ade80', boxShadow: '0 0 6px #4ade80', flexShrink: 0 }} />
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: '#D4A843', letterSpacing: '0.12em', fontWeight: 600 }}>
                  {properties.length} {tx.stays_listed}
                </span>
              </div>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fill, minmax(${isMobile ? '100%' : '300px'}, 1fr))`, gap: isMobile ? 16 : 24 }}>
            {filtered.map(p => {
              ...
              return (
                <article key={p.id}
                  onClick={() => router.push(`/${locale}/stays/${p.slug || p.id}`)}
