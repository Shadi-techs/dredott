  ...
  async function fetchPendingListings() {
    ...
    try {
      const [propertiesRes, carsRes] = await Promise.all([
        ...
        supabase
          .from('cars')
          .select(`id, name, slug, area, price_per_day, brand, model, year, seats, photos, review_status, created_at, owner:profiles!owner_user_id (id, first_name, last_name, email, phone)`)
          .eq('review_status', 'pending_review')
          .order('created_at', { ascending: false }),
      ])
      ...
      <div className="border-b border-white/10 bg-[#1a2240]">
        <div className="px-6 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2 bg-[#0e1428] rounded-lg px-4 py-2 border border-white/10">
      ...
      <div className="p-6">
        {loading ? (
          ...
          <div className="text-center py-20">
            <AlertCircle className="w-12 h-12 text-[#7a8aaa] mx-auto mb-4" />
            <p className="text-[#7a8aaa] text-lg">
...
function ListingCard({
  ...
  return (
    <div className="bg-[#1a2240] rounded-lg border border-white/10 overflow-hidden hover:border-[#D4A843]/30 transition-colors">
      <div className="relative h-48 bg-[#0e1428]">
        <Image src={mainPhoto} alt={listing.name} fill className="object-cover" />
        <div className="absolute top-3 left-3">
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#0e1428]/80 backdrop-blur-sm border border-white/10">
      ...
      <div className="p-4">
        <h3 className="text-lg font-semibold text-[#FBF0D0] mb-2 line-clamp-1">{listing.name}</h3>

        <div className="flex items-center gap-2 text-sm text-[#7a8aaa] mb-3">
        ...
        <div className="flex items-center gap-4 mb-3 pb-3 border-b border-white/10">
          {isProperty ? (
            <>
              ...
              <div className="text-sm text-[#7a8aaa]">
                {listing.bedrooms || 0} bed • {listing.bathrooms || 0} bath
              </div>
            </>
          ) : (
            <>
              <div className="text-sm text-[#7a8aaa]">{listing.brand} {listing.model}</div>
              <div className="flex items-center gap-1.5 text-sm text-[#7a8aaa]">
                <Users className="w-4 h-4" />
                <span>{listing.seats || 0} seats</span>
              </div>
              <div className="text-sm text-[#7a8aaa]">{listing.year}</div>
            </>
          )}
        </div>

        <div className="mb-3">
          <p className="text-xs text-[#7a8aaa] mb-0.5">Owner</p>
          <p className="text-sm text-[#FBF0D0] font-medium">
            {listing.owner.first_name} {listing.owner.last_name}
          </p>
          <p className="text-xs text-[#7a8aaa]">{listing.owner.email}</p>
        </div>
        ...
        <div className="mb-3">
          <p className="text-2xl font-bold text-[#D4A843]">
            ...
            <span className="text-sm text-[#7a8aaa] font-normal">/{isProperty ? 'night' : 'day'}</span>
          </p>
        </div>

        <div className="flex items-center gap-2 text-xs text-[#7a8aaa] mb-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={onView} className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-[#0e1428] border border-white/10 text-[#7a8aaa] hover:text-[#FBF0D0] hover:border-[#D4A843]/30 transition-colors text-sm">
...
function ReviewModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1a2240] rounded-lg border border-white/10 max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-[#7a8aaa]">{listing.name}</p>
          </div>
        </div>
        ...
        {needsReason && presets.length > 0 && (
          <div className="mb-4">
            <label className="block text-sm font-medium text-[#FBF0D0] mb-2">
              Choose a reason
            </label>
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 rounded-lg border text-sm transition-colors ${
                    selectedPreset === preset.id
                      ...
                      : 'bg-[#0e1428] border-white/10 text-[#7a8aaa] hover:text-[#FBF0D0] hover:border-white/20'
                  }`}
                >
                  {preset.reason_en}
                  {preset.reason_ar && (
        ...
        <div className="mb-6">
          <label className="block text-sm font-medium text-[#FBF0D0] mb-2">
            Internal note <span className="text-[#7a8aaa] font-normal">(not sent to owner)</span>
          </label>
          <textarea
            value={internalNote}
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            disabled={submitting}
            className="flex-1 px-4 py-2 rounded-lg border border-white/10 text-[#7a8aaa] hover:text-[#FBF0D0] transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
