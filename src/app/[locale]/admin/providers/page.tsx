      ...
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
          {['pending', 'approved', 'rejected', 'all'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ padding: '6px 16px', borderRadius: 100, fontSize: 12, cursor: 'pointer', border: '1px solid', background: filter === f ? '#D4A843' : 'transparent', color: filter === f ? '#0e1428' : '#D4A843', borderColor: '#D4A843', fontWeight: filter === f ? 600 : 400, textTransform: 'capitalize' }}>
              {f}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {providers.map(p => (
              <div key={p.id} style={{ background: '#1a2240', borderRadius: 12, padding: 20, border: '1px solid rgba(255,255,255,0.08)' }}>
