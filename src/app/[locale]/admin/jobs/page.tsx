      ...
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
          {['pending_review', 'live', 'rejected', 'all'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ padding: '6px 16px', borderRadius: 100, fontSize: 12, cursor: 'pointer', border: '1px solid', background: filter === f ? '#D4A843' : 'transparent', color: filter === f ? '#F0F2F7' : '#D4A843', borderColor: '#D4A843', fontWeight: filter === f ? 600 : 400 }}>
              {f.replace('_', ' ')}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {jobs.map(job => (
              <div key={job.id} style={{ background: '#1e2d4f', borderRadius: 12, padding: 20, border: '1px solid rgba(255,255,255,0.08)' }}>
