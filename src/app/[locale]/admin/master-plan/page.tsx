...
export default function MasterPlanPage() {
  ...
  if (loading) return <div style={{ minHeight: '100vh', background: '#F0F2F7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 32, height: 32, border: '3px solid #D4A843', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>

  if (!authorized) return <div style={{ minHeight: '100vh', background: '#F0F2F7', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}><Lock size={40} color="#D4A843" /><p style={{ color: '#FBF0D0', fontFamily: "'Cormorant Garamond', serif", fontSize: 24 }}>Super Admin Only</p></div>
  ...
  return (
    <>
      ...
      <div className="print-page" style={{ minHeight: '100vh', background: '#F0F2F7', padding: 32, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="no-print" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
          ...
          <button onClick={() => window.print()} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', background: '#D4A843', color: '#F0F2F7', border: 'none', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: 13 }}><Download size={15} />PDF</button>
        </div>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <S title="01 · Brand & Domain">
            ...
            <R label="Colors" value="#F0F2F7 Navy · #D4A843 Gold · #FAF9F6 Cream · #2A9D8F Teal" />
