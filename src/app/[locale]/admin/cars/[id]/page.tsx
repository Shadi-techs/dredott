...
import {
  ...
  Users, DollarSign, Calendar, Car,
  Zap, AlertTriangle, Info, Sparkles, RotateCcw,
} from 'lucide-react'

// ── Moderation modal ─────────────────────────────────────────
function ModerationModal({ action, listing, onClose, onDone }: {
  ...
  const [customReason, setCustom]     = useState(listing._prefill || '')
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/moderation/queue?presets=1&type=car&decision=${action}`)
  ...
  async function submit() {
    ...
    try {
      ...
      const body: any = { entity_type: 'car', entity_id: listing.id, internal_note: internalNote || undefined }
      if (action !== 'approve') { body.reason_preset_id = selectedPreset || undefined; body.reason_custom = customReason || undefined }
          ...
          <>
            {presets.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                ...
                <select value={selectedPreset} onChange={e => setSelected(e.target.value)} style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e5e7eb', fontSize: 13 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 12, color: '#6B7280', display: 'block', marginBottom: 6 }}>CUSTOM REASON</label>
              <textarea value={customReason} onChange={e => setCustom(e.target.value)} rows={3} placeholder="Write a reason for the owner…" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e5e7eb', fontSize: 13, resize: 'vertical', fontFamily: 'inherit' }} />
            </div>
          </>
        )}
        <div style={{ marginBottom: 20 }}>
          <label style={{ fontSize: 12, color: '#6B7280', display: 'block', marginBottom: 6 }}>INTERNAL NOTE</label>
...
// ── AI Review Panel ──────────────────────────────────────────
function AiReviewPanel({ listingId, onUseSuggestion }: { listingId: string; onUseSuggestion: (text: string) => void }) {
  ...
  async function runReview() {
    ...
    try {
      const res = await fetch('/api/admin/ai-review', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ listing_type: 'car', listing_id: listingId }) })
    ...
    <div style={{ background: 'linear-gradient(135deg,#0e1428,#1a2240)', borderRadius: 16, padding: 20, border: '1px solid rgba(212,168,67,0.2)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(212,168,67,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Sparkles size={16} color="#D4A843" /></div>
          <div>
      ...
      {!result && !loading && <p style={{ fontSize: 13, color: 'rgba(201,206,221,0.5)', textAlign: 'center', padding: '16px 0' }}>Click "Run Analysis" for AI feedback</p>}
      {loading && <p style={{ fontSize: 13, color: 'rgba(201,206,221,0.6)', textAlign: 'center', padding: '16px 0' }}>Analyzing listing…</p>}
        ...
        <>
          ...
          {result.issues?.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              ...
              {result.issues.map((issue: any, i: number) => dismissed.has(i) ? null : (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', background: 'rgba(255,255,255,0.04)', borderRadius: 8, marginBottom: 6 }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: sevColor(issue.severity), flexShrink: 0, marginTop: 5 }} />
                  <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    {!applied.has(i) ? <button onClick={() => { onUseSuggestion(issue.message); setApplied(s => new Set(s).add(i)) }} style={{ fontSize: 10, padding: '4px 8px', borderRadius: 6, border: '1px solid rgba(212,168,67,0.3)', background: 'rgba(212,168,67,0.1)', color: '#D4A843', cursor: 'pointer', whiteSpace: 'nowrap' }}>Use</button>
                    : <span style={{ fontSize: 10, color: '#4ade80' }}>✓ Used</span>}
          ...
          {result.suggestions?.length > 0 && (
            <div>
              ...
              {result.suggestions.map((s: string, i: number) => dismissed.has(100 + i) ? null : (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', background: 'rgba(255,255,255,0.03)', borderRadius: 8, marginBottom: 6 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    {!applied.has(100 + i) ? <button onClick={() => { onUseSuggestion(s); setApplied(a => new Set(a).add(100 + i)) }} style={{ fontSize: 10, padding: '4px 8px', borderRadius: 6, border: '1px solid rgba(212,168,67,0.3)', background: 'rgba(212,168,67,0.1)', color: '#D4A843', cursor: 'pointer', whiteSpace: 'nowrap' }}>Use</button>
                    : <span style={{ fontSize: 10, color: '#4ade80' }}>✓ Used</span>}
...
export default function AdminCarDetailPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/cars/${id}`)
      .then(r => r.json()).then(d => { setCar(d.car); setLoading(false) }).catch(() => setLoading(false))
  }, [id])
  ...
  const sc = statusColors[car.review_status] || statusColors.pending_review
  const isPending = car.review_status === 'pending_review'
    ...
    <div style={{ maxWidth: 960, margin: '0 auto', padding: 28 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: '#F0F2F7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Car size={22} color="#1a2240" /></div>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1a2240', margin: 0 }}>{car.name}</h1>
            <p style={{ fontSize: 13, color: '#6B7280', margin: '3px 0 0' }}>{car.brand} {car.model} · {car.year}</p>
          </div>
        </div>
        <span style={{ padding: '6px 14px', borderRadius: 100, fontSize: 12, fontWeight: 600, background: sc.bg, color: sc.color }}>
          {car.review_status?.replace(/_/g, ' ')}
        </span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24, alignItems: 'start' }}>
        <div>
          ...
          {car.photos?.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 4, borderRadius: 14, overflow: 'hidden', marginBottom: 20 }}>
              <img src={car.photos[0]} alt="" style={{ width: '100%', height: 280, objectFit: 'cover' }} />
              <div style={{ display: 'grid', gridTemplateRows: '1fr 1fr', gap: 4 }}>
                {car.photos.slice(1, 3).map((p: string, i: number) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
            <div style={{ background: '#F4F6FA', borderRadius: 12, padding: 16 }}>
              <p style={{ fontSize: 10, color: '#9CA3AF', fontFamily: 'monospace', letterSpacing: '0.15em', margin: '0 0 10px' }}>CAR DETAILS</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div style={{ fontSize: 13, color: '#1a2240' }}>⚙️ {car.transmission || '—'}</div>
                <div style={{ fontSize: 13, color: '#1a2240' }}>⛽ {car.fuel_type || '—'}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#1a2240' }}><Users size={14} color="#9CA3AF" /> {car.seats || '—'} seats</div>
                {car.doors && <div style={{ fontSize: 13, color: '#1a2240' }}>🚪 {car.doors} doors</div>}
              </div>
            </div>
            <div style={{ background: '#F4F6FA', borderRadius: 12, padding: 16 }}>
              <p style={{ fontSize: 10, color: '#9CA3AF', fontFamily: 'monospace', letterSpacing: '0.15em', margin: '0 0 10px' }}>PRICING</p>
              <div style={{ fontSize: 26, fontWeight: 800, color: '#D4A843', marginBottom: 6 }}>EGP {car.price_per_day?.toLocaleString()}<span style={{ fontSize: 13, fontWeight: 400, color: '#9CA3AF' }}>/day</span></div>
              {car.price_per_week  && <div style={{ fontSize: 12, color: '#6B7280' }}>EGP {car.price_per_week?.toLocaleString()} / week</div>}
              {car.price_per_month && <div style={{ fontSize: 12, color: '#6B7280' }}>EGP {car.price_per_month?.toLocaleString()} / month</div>}
            </div>
          </div>
          ...
          {car.description && (
            <div style={{ background: '#F4F6FA', borderRadius: 12, padding: 16, marginBottom: 20 }}>
              ...
              <p style={{ fontSize: 14, color: '#1a2240', lineHeight: 1.75, margin: 0 }}>{car.description}</p>
            </div>
          )}
          ...
          {car.owner && (
            <div style={{ background: '#F4F6FA', borderRadius: 12, padding: 16, marginBottom: 20 }}>
              ...
              <p style={{ fontSize: 15, fontWeight: 700, color: '#1a2240', margin: '0 0 4px' }}>{car.owner.first_name} {car.owner.last_name}</p>
              {car.owner.email && <p style={{ fontSize: 13, color: '#6B7280', margin: '0 0 3px' }}>✉️ {car.owner.email}</p>}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#9CA3AF' }}>
            <Calendar size={13} />
            Submitted {new Date(car.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <AiReviewPanel listingId={id} onUseSuggestion={(text) => setPrefill(text)} />
          ...
          {prefill && (
            <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 10, padding: 14 }}>
              ...
              <p style={{ fontSize: 11, color: '#B45309', margin: '8px 0 0' }}>Will be pre-filled when you click "Request Changes"</p>
            </div>
          )}
        </div>
      </div>
