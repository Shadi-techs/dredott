...
export default function AdminSidebar({ user, locale }: { user: AdminUser; locale: string }) {
  ...
  return (
    <>
      ...
      <div
        className={`
          hidden lg:flex flex-col bg-[#1a2240] flex-shrink-0
      ...
      <button
        ...
        className="lg:hidden fixed top-3 left-3 z-50 bg-[#1a2240] border border-[#D4A843]/20 text-[#D4A843] p-2 rounded-lg shadow-lg"
      >
      ...
      {mobileOpen && (
        <>
          <div
            ...
            onClick={() => setMobileOpen(false)}
          />
          <div className="fixed left-0 top-0 bottom-0 w-[210px] bg-[#1a2240] z-50 lg:hidden border-r border-[#D4A843]/10">
            <SidebarContent />
          </div>
        </>
      )}
